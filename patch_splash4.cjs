...
const targetPath = path.join(process.cwd(), 'src/components/SplashScreen.tsx');
let code = fs.readFileSync(targetPath, 'utf8');

const regex = /\{\/\* Hero Content \*\/\}[\s\S]*?(?=\{\/\* Footer Decoration \*\/\}|$)/;
const match = code.match(regex);

if (match) {
  const newContent = `{/* Hero Content & Queue Layout */}
