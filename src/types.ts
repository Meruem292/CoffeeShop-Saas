export type ViewMode = 'pos' | 'kiosk' | 'mobile' | 'queue' | 'inventory' | 'admin-products' | 'admin-vouchers' | 'admin-customers' | 'admin-reviews' | 'settings' | 'cashier' | 'reports' | 'profile' | 'order-history' | 'rewards-store' | 'admin-chat' | 'customer-chat' | 'admin-yourmix';
...
export interface SplashScreen {
  ...
  glbPositionY?: number;
  glbRotationY?: number;
  glbCameraPitch?: number;
  glbAutoRotate?: boolean;
}
...
export interface Review {
  ...
  isApproved: boolean; // Must be approved by admin to show on splash
  isFeatured?: boolean;
  createdAt: number;
  updatedAt: number;
}
