...
import { Coffee, ArrowRight, ChefHat, CheckCircle2, Clock, Lock, Zap, Sparkles, Orbit, Flame, Star, ChevronLeft, ChevronRight, User, Quote, MessageSquareQuote } from 'lucide-react';
...
interface SplashScreenProps {
  ...
  onStart: (recognizedCustomer?: UserProfile) => void;
...
export function SplashScreen({ 
  ...
  // Filter approved reviews for the slideshow
  ...
  React.useEffect(() => {
    if (approvedReviews.length <= 1 || isPaused) return;
  ...
  }, [approvedReviews.length, isPaused]);
  ...
  const glbScale = data?.glbScale ?? 1.0;
  const glbZoom = data?.glbZoom ?? 100;
  const glbPositionX = data?.glbPositionX ?? 0;
  const glbPositionY = data?.glbPositionY ?? 0;
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-8 h-8 rounded-xl overflow-hidden bg-slate-800 border border-white/10 shrink-0 flex items-center justify-center">
                      {currentReview.userPhoto ? (
                        ...
                        <div className="w-full h-full bg-gradient-to-tr from-amber-600 to-amber-400 flex items-center justify-center text-slate-950 font-black text-xs">
                          {currentReview.userName ? currentReview.userName.slice(0, 2).toUpperCase() : <User className="w-3.5 h-3.5" />}
                        </div>
                      )}
                    </div>
          ...
          <div className="absolute right-0 top-0 bottom-0 w-full lg:w-1/2 pointer-events-none opacity-40 lg:opacity-100 flex items-center justify-center z-0 overflow-hidden">
            {data?.useGlb && !hasModelError ? (
              <div 
                ...
                style={{
                  transform: `translate(${glbPositionX}%, ${glbPositionY}%)`,
              ...
              >
                <model-viewer
                  ...
                  alt="3D Splash Model"
                  {...(glbAutoRotate ? { 'auto-rotate': true } : {})}
                  camera-controls
                  ar
                  style={{ width: '100%', height: '100%', backgroundColor: 'transparent' }}
                  scale={`${glbScale} ${glbScale} ${glbScale}`}
                  camera-orbit={`${glbRotationY}deg ${glbCameraPitch}deg ${glbZoom}%`}
