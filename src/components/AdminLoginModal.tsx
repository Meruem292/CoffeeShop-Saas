...
export function AdminLoginModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="bg-white dark:bg-[#0a0a0c] rounded-[3rem] p-10 max-w-sm w-full shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] relative border-2 border-black/10 dark:border-white/5 z-10 backdrop-blur-xl animate-in zoom-in-95 duration-500"
      >
        <button 
          onClick={onClose}
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="w-16 h-16 bg-black/5 dark:bg-white/5 text-amber-500 rounded-2xl flex items-center justify-center mb-6 shadow-inner border border-black/10 dark:border-white/10">
            <Store className="w-8 h-8" />
          </div>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white text-center uppercase italic tracking-tighter">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full p-4 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl focus:border-amber-500/50 outline-none text-slate-900 dark:text-white font-bold transition-all placeholder:text-coffee-900" 
