  ...
  return (
    <MagicBento 
      ...
      className={`flex flex-col h-full overflow-hidden rounded-[1.75rem] border transition-all duration-300 shadow-[0_8px_32px_rgba(0,0,0,0.15)] ${
        isAvailable 
          ? 'cursor-pointer group/card border-white/5 bg-slate-900/20 backdrop-blur-xl hover:bg-slate-900/40 hover:border-white/10' 
    ...
    >
      ...
      >
        ...
        <div className={`flex flex-col flex-1 px-4 pb-4 pt-1 text-left ${!isAvailable ? 'opacity-50' : ''}`}>
          <h3 className={`font-display font-bold text-white tracking-tight leading-tight transition-colors ${isAvailable ? 'group-hover/card:text-amber-400' : ''} ${mode === 'mobile' ? 'text-xs mb-1' : 'text-sm md:text-base mb-1.5'}`}>
            {item.name}
          </h3>
          ...
          <div className="mt-auto pt-1.5 flex items-center justify-between border-t border-white/5">
            <div className="flex flex-col">
              ...
              <span className={`font-display font-bold ${isAvailable ? 'text-amber-400' : 'text-slate-400'} ${mode === 'mobile' ? 'text-sm' : 'text-base'}`}>
            ...
            {isAvailable ? (
              ...
              <div className="w-7 h-7 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center text-slate-600">
