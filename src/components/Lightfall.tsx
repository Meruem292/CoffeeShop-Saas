...
const Lightfall = ({
  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      ...
      const dt = (time - lastTimeRef.current) * 0.001;
      lastTimeRef.current = time;

      uniforms.iTime.value += dt;

      if (mouseInteraction) {
        uniforms.iMouse.value[0] += (mouseTargetRef.current[0] - uniforms.iMouse.value[0]) * mouseDampening;
        uniforms.iMouse.value[1] += (mouseTargetRef.current[1] - uniforms.iMouse.value[1]) * mouseDampening;
      }
