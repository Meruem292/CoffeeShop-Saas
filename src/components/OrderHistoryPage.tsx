...
import { ShoppingBag, Clock, Receipt, ChevronDown, ChevronUp, User, Tag, Award, Search, Filter, ArrowLeft, MessageSquare, Star, MessageSquareQuote, CheckCircle2, Sparkles } from 'lucide-react';
import { Order, ViewMode, Review, UserProfile } from '../types';
...
export function OrderHistoryPage({ 
  ...
  const userReview = reviews?.find(r => r.userId === currentUserUid || r.id === currentUserUid);
  ...
  return (
    <div className="p-3 sm:p-6 md:p-8 max-w-4xl mx-auto space-y-5 sm:space-y-6 overflow-y-auto pb-24">
      ...
      {/* Customer Review Invitation / Status Banner */}
      {onSubmitReview && (
        <div className="bg-white dark:bg-[#0a0a0c] p-4 sm:p-5 rounded-2xl border border-black/10 dark:border-white/5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 relative overflow-hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h4 className="text-xs sm:text-sm font-black uppercase italic text-slate-900 dark:text-white">
                  {userReview ? 'Your Customer Review' : 'Customer Review Spotlight'}
                </h4>
                {userReview?.isApproved && (
                  <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 flex items-center gap-1">
                    <CheckCircle2 className="w-2.5 h-2.5" /> Live on Splash
                  </span>
                )}
                {userReview && !userReview.isApproved && (
                  <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-500 border border-amber-500/30 flex items-center gap-1 animate-pulse">
              ...
              <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                {completedCount >= 3
                  ? userReview 
                    ? `"${userReview.comment.slice(0, 75)}${userReview.comment.length > 75 ? '...' : ''}"`
                    : `You have completed ${completedCount} orders! Share your experience on our splash screen.`
                  : `Complete 3 orders to unlock customer reviews (${completedCount}/3 completed).`}
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className={`w-full sm:w-auto px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all shrink-0 ${
              completedCount >= 3
                ? 'bg-amber-500 hover:bg-amber-400 text-slate-950 shadow-md shadow-amber-500/20'
                : 'bg-black/5 dark:bg-white/5 text-slate-500 border border-black/10 dark:border-white/10'
            }`}
          >
            <MessageSquareQuote className="w-3.5 h-3.5" />
            {userReview ? 'Edit Review' : completedCount >= 3 ? 'Write Review' : 'Requirements'}
          </button>
        </div>
      )}
