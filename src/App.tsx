...
import { ViewMode, Order, Product, OrderStatus, UserProfile } from './types';
import { SplashScreen } from './components/SplashScreen';
import { UnifiedAuthModal } from './components/UnifiedAuthModal';
import { Store, MonitorSmartphone, Tablet, Smartphone, ChefHat, Package, CheckCircle2, Settings, LogOut, ShieldAlert, Lock, Home, Banknote, BarChart3, Sun, Moon, Search, X, Coffee, Croissant, CakeSlice, Cookie, Milk, CupSoda, Utensils, Menu, ChevronRight , Tag, User, Coins, Download, ShoppingBag, MessageSquare, FlaskConical, Star, Clock, AlertTriangle } from 'lucide-react';
...
export default function App() {
  ...
  }, [successOrder, currentView]);

  // Kiosk Inactivity Reset Timer (60s total: 45s silent + 15s warning popup)
  useEffect(() => {
    if (!isKioskModeActive || !isStarted || currentView !== 'kiosk') {
      setShowKioskIdleWarning(false);
      return;
    }
    ...
    const resetIdleTimer = () => {
      setShowKioskIdleWarning(false);
      setKioskIdleCountdown(15);
      clearTimeout(idleTimeout);
      clearInterval(countdownInterval);

      // After 45s of inactivity, show 15s warning modal
      idleTimeout = setTimeout(() => {
        setShowKioskIdleWarning(true);
        let count = 15;
        countdownInterval = setInterval(() => {
          count -= 1;
          setKioskIdleCountdown(count);
          if (count <= 0) {
            clearInterval(countdownInterval);
            setKioskCustomerProfile(null);
            setIsStarted(false);
            setShowKioskIdleWarning(false);
            toast.info('Kiosk session reset due to inactivity.');
          }
        }, 1000);
      }, 45000);
    };
  ...
  }, [isKioskModeActive, isStarted, currentView]);
  ...
  const navigationItems: { id: ViewMode; label: string; icon: React.ReactNode; adminOnly?: boolean; userOnly?: boolean; customerOnly?: boolean }[] = [
    ...
    { id: 'admin-reviews', label: 'Reviews', icon: <Star className="w-4 h-4" />, adminOnly: true },
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [successOrder, isKioskModeActive]);
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-white font-sans selection:bg-amber-500/30 overflow-hidden relative">
      ...
      <div className="flex-1 flex min-h-screen overflow-hidden relative z-10 w-full">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          {item.id === 'admin-reviews' && pendingReviewsCount > 0 && (
                            <span className="ml-2 px-2 py-0.5 text-[10px] font-black bg-amber-500 text-slate-950 rounded-full shadow-lg shadow-amber-500/40 font-mono">
                              {pendingReviewsCount}
                            </span>
                          )}
                          {((item.id === 'cashier' && (unpaidOrdersCount > 0 || pendingVerificationOrdersCount > 0)) || (item.id === 'queue' && pendingOrdersCount > 0)) && (
                  ...
                  {user ? (
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-3 p-2 rounded-lg bg-black/5 dark:bg-white/5">
                        <div className="w-8 h-8 rounded-lg bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-500 font-bold text-[10px] shrink-0 overflow-hidden shadow-inner">
                          {userProfile?.photoURL || user.photoURL ? (
                            <img 
                              src={userProfile?.photoURL || user.photoURL} 
                              alt={userProfile?.displayName || user.email || 'User'} 
                    ...
                    <>
                      ...
                      {currentView === 'admin-reviews' && (
                        <AdminReviews
                          ...
                          onToggleFeatureReview={toggleFeatureReview}
        ...
        {/* Kiosk Inactivity Reset Warning Modal */}
        {showKioskIdleWarning && isKioskModeActive && isStarted && currentView === 'kiosk' && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-2xl animate-in fade-in duration-300">
            <div className="bg-[#0e0f14] border border-amber-500/30 rounded-[2.5rem] p-8 max-w-md w-full shadow-[0_20px_70px_rgba(245,158,11,0.25)] text-center relative overflow-hidden animate-in zoom-in-95 duration-300">
              {/* Pulsing ambient glow */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-60 h-32 bg-amber-500/15 rounded-full blur-[50px] pointer-events-none" />

              <div className="relative mx-auto mb-6 w-20 h-20 rounded-3xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-400 shadow-[0_0_30px_rgba(245,158,11,0.2)]">
                <Clock className="w-10 h-10 animate-spin text-amber-400" style={{ animationDuration: '6s' }} />
                <span className="absolute text-xs font-black text-amber-300 tracking-tighter">{kioskIdleCountdown}</span>
              </div>

              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[11px] font-black uppercase tracking-wider mb-3">
                <AlertTriangle className="w-3.5 h-3.5" /> Inactivity Notice
              </div>

              <h3 className="text-2xl font-black text-white uppercase italic tracking-tight mb-2">
                Still Ordering{kioskCustomerProfile?.displayName ? `, ${kioskCustomerProfile.displayName}` : ''}?
              </h3>
              <p className="text-white/60 text-xs leading-relaxed mb-6 font-medium">
                Your session will automatically conclude and reset in{' '}
                <span className="text-amber-400 font-bold text-sm">{kioskIdleCountdown} seconds</span> to protect account privacy.
              </p>
              ...
              <div className="space-y-3">
                <button
                  ...
                  className="w-full py-4 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-black text-xs uppercase tracking-[0.2em] shadow-lg shadow-amber-500/25 active:scale-95 transition-all"
                >
                  Keep Ordering (I'm Here)
                </button>
                ...
                <button
                  onClick={() => {
                    ...
                    setKioskCustomerProfile(null);
                    setIsStarted(false);
                    toast.info('Session ended. Have a great day!');
                  }}
                  className="w-full py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white/50 hover:text-white text-xs font-bold uppercase tracking-wider transition-colors"
                >
                  End Session Now
                </button>
              </div>
