...
export async function detectHeadPoseAndExpression(
  ...
  try {
    ...
    // Depth difference between eyes (3D Z coordinates if available)
    const eyeZDiff = pLeftEye.z !== undefined && pRightEye.z !== undefined
      ? (pRightEye.z - pLeftEye.z) / eyeWidth
      : 0;

    // Clean thresholds:
    // Center: looking directly forward at the camera
    const isCenter = Math.abs(noseOffset) < 0.065 && Math.abs(symmetryRatio) < 0.075;

    // Turn Left / Turn Right:
    // Natural head rotation to left or right (responsive threshold ~10-25 degrees)
    const isTurnLeft = !isCenter && (noseOffset > 0.065 || symmetryRatio > 0.075 || eyeZDiff > 0.075);
    const isTurnRight = !isCenter && (noseOffset < -0.065 || symmetryRatio < -0.075 || eyeZDiff < -0.075);
...
/**
 * Extract all available normalized biometric vectors for a customer profile across all angles
 */
export function extractAllProfileVectors(profile: any): number[][] {
  if (!profile) return [];
  const vectors: number[][] = [];

  if (Array.isArray(profile.faceVector_front) && profile.faceVector_front.length > 0) {
    vectors.push(profile.faceVector_front);
  }
  if (Array.isArray(profile.faceVector_left) && profile.faceVector_left.length > 0) {
    vectors.push(profile.faceVector_left);
  }
  if (Array.isArray(profile.faceVector_right) && profile.faceVector_right.length > 0) {
    vectors.push(profile.faceVector_right);
  }
  if (Array.isArray(profile.faceVector_smile) && profile.faceVector_smile.length > 0) {
    vectors.push(profile.faceVector_smile);
  }

  if (profile.faceAngles && typeof profile.faceAngles === 'object') {
    Object.values(profile.faceAngles).forEach((vec: any) => {
  ...
  if (profile.faceVectors) {
    const parsed = parseStoredFaceVectors(profile.faceVectors);
    parsed.forEach((vec) => {
      if (Array.isArray(vec) && vec.length > 0) {
        vectors.push(vec);
      }
...
/**
 * Find the best matched candidate profile among a list of candidates
 */
export function findBestMatchedProfile(
  liveVector: number[],
  candidates: any[],
  minConfidence: number = 65
): { matchedProfile: any | null; confidence: number; similarity: number } {
  if (!liveVector || liveVector.length === 0 || !candidates || candidates.length === 0) {
    return { matchedProfile: null, confidence: 0, similarity: 0 };
  }
  ...
  for (const candidate of candidates) {
    const candidateVectors = extractAllProfileVectors(candidate);
    if (candidateVectors.length === 0) continue;

    for (const vec of candidateVectors) {
      const { confidence, similarity } = calculateFaceMatchConfidence(liveVector, vec);
  ...
  if (highestConfidence >= minConfidence && bestCandidate) {
    return {
      matchedProfile: bestCandidate,
      confidence: highestConfidence,
      similarity: highestSimilarity,
    };
  }

  return { matchedProfile: null, confidence: highestConfidence, similarity: highestSimilarity };
}
