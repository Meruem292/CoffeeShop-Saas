...
export function useFirebase(userUid?: string, isAdmin?: boolean) {
  ...
  const submitReview = async (reviewData: { rating: number; comment: string; userName?: string; userPhoto?: string }) => {
    if (!userUid) {
      toast.error('Please log in to submit a review');
      return false;
    }

    // Verify completed order count (at least 3 completed transactions)
    const completedCount = userOrders.filter(o => o.status === 'completed').length;
    if (completedCount < 3) {
      toast.error('You need at least 3 completed transactions to submit a review.');
      return false;
    }
    ...
    if (!reviewData.comment || reviewData.comment.trim().length < 3) {
      toast.error('Please write a short review (at least 3 characters)');
      return false;
    }

    try {
      const existing = reviews.find(r => r.userId === userUid || r.id === userUid);
      const reviewDoc: Review = {
        id: userUid,
        userId: userUid,
        userName: reviewData.userName || userProfile?.displayName || auth.currentUser?.displayName || 'Valued Customer',
        userEmail: userProfile?.email || auth.currentUser?.email || undefined,
        userPhoto: reviewData.userPhoto || userProfile?.photoURL || auth.currentUser?.photoURL || undefined,
        rating: Math.min(Math.max(Math.round(reviewData.rating), 1), 5),
        comment: reviewData.comment.trim(),
        completedOrdersCount: completedCount,
        isApproved: false, // Explicit requirement: Requires Admin approval to avoid explicit reviews
        isFeatured: existing ? !!existing.isFeatured : false,
        createdAt: existing ? existing.createdAt : Date.now(),
        updatedAt: Date.now()
      };

      await setDoc(doc(db, 'reviews', userUid), reviewDoc);
      toast.success(existing ? 'Review updated! Awaiting admin approval.' : 'Review submitted! Awaiting admin approval.');
      return true;
    } catch (err) {
      handleFirestoreError(err, OperationType.WRITE, `reviews/${userUid}`);
  ...
  const approveReview = async (reviewId: string, isApproved: boolean) => {
    try {
      ...
      toast.success(isApproved ? 'Review approved for Splash Screen slideshow!' : 'Review unapproved / hidden.');
      return true;
    } catch (err) {
  ...
  const toggleFeatureReview = async (reviewId: string, isFeatured: boolean) => {
    try {
      await updateDoc(doc(db, 'reviews', reviewId), {
        isFeatured,
        updatedAt: Date.now()
      });
      toast.success(isFeatured ? 'Review marked as featured!' : 'Review unfeatured.');
      return true;
    } catch (err) {
      handleFirestoreError(err, OperationType.UPDATE, `reviews/${reviewId}`);
